...
def preprocess(args):
    ...
    prepro_each(args, ".../train-test_data/data_train.json", 0.0, 1.0,  out_name='train')
    prepro_each(args, ".../train-test_data/data_test.json", 0.0, 1.0, out_name='test')


def save(data, shared, out_name):
    data_path = os.path.join(".../train-test_data", "data_{}.json".format(out_name))
    shared_path = os.path.join(".../train-test_data", "shared_{}.json".format(out_name))
...
def prepro_each(args, data_path, start_ratio, stop_ratio, out_name):
    # Choose tokenizer, jieba is the original embedding txt for Chinese task, thus change to nltk for English
    ...
    for ai, content in enumerate(tqdm(source_data[start_ai:stop_ai])):
        ...
        # resume_exp
        if isinstance(resume_list[0], list):# check whether this is a list
...
def preprocess_Infer(args):
    ...
    prepro_Infer_each(args, ".../train-test_data/data_train.json", "train")
    prepro_Infer_each(args, ".../train-test_data/data_test.json", "test")
    prepro_Infer_each(args, ".../train-test_data/data_dev.json", "dev")


def prepro_Infer_each(args, data_path, out_name, exp_len = 15):
    # Choose tokenizer
    if args.tokenizer == 'jieba':
        word_tokenizer = jieba.cut
    elif args.tokenizer == 'nltk':
    ...
    jd_write_file = open(".../s1.{}".format(out_name), 'w', encoding='utf8')
    cv_write_file = open(".../s2.{}".format(out_name), 'w', encoding='utf8')
    label_write_file = open(".../labels.{}".format(out_name), 'w', encoding='utf8')
    ...
    for i, content in enumerate(tqdm(source_data[0:len(source_data)])):
        ...
        # write, only longer resume will be writen
...
def preprocess_Graph(args):
    ...
    prepro_Graph_each(args, ".../train-test_data/data_train.json", "train")
    prepro_Graph_each(args, ".../train-test_data/data_test.json", "test")
    prepro_Graph_each(args, ".../train-test_data/data_dev.json", "dev")


def prepro_Graph_each(args, data_path, out_name, exp_len = 15, graph_num = 5):
    # Choose tokenizer
    if args.tokenizer == 'jieba':
        word_tokenizer = jieba.cut
    elif args.tokenizer == 'nltk':
    ...
    # Reading successful result
    with open(".../graph_user.json", 'r', encoding='utf8') as f:
        user_luqu_dict = json.load(f)
    with open(".../graph_jd.json", 'r', encoding='utf8') as f:
        jd_luqu_dict = json.load(f)

    # Reading nothired.json, not used in current task
    with open(".../graph_nothired_user.json", 'r', encoding='utf8') as f:
        user_nothired_dict = json.load(f)
    with open(".../graph_nothired_jd.json", 'r', encoding='utf8') as f:
        jd_nothired_dict = json.load(f)

    jd_write_file = open(".../s1.{}".format(out_name), 'w', encoding='utf8')
    cv_write_file = open(".../s2.{}".format(out_name), 'w', encoding='utf8')
    label_write_file = open(".../labels.{}".format(out_name), 'w', encoding='utf8')
    jd_graph_file = open(".../s1_graph.{}".format(out_name), 'w', encoding='utf8')
    cv_graph_file = open(".../s2_graph.{}".format(out_name), 'w', encoding='utf8')
...
def main():
    ...
    parser.add_argument('--table_action', default='.../action.csv')
    parser.add_argument('--f_save_uid2num', default='.../uid2num.json')
    parser.add_argument('--f_save_unum2id', default='.../unum2id.json')
    parser.add_argument('--f_save_jid2num', default='.../jid2num.json')
    parser.add_argument('--f_save_jnum2id', default='.../jnum2id.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--orignal_data_dir', default='D:/FTEC5910/')
    parser.add_argument('--tokenizer', default='nltk')
    parser.add_argument('--train_test_dir', default='D:/FTEC5910/cvjd_parser/train-test_data/')
    parser.add_argument('--table_action',
                        default='.../action.csv')
    parser.add_argument('--f_save_uid2num',
                        default='.../uid2num.json')
    parser.add_argument('--f_save_unum2id',
                        default='.../unum2id.json')
    parser.add_argument('--f_save_jid2num',
                        default='.../jid2num.json')
    parser.add_argument('--f_save_jnum2id',
                        default='.../jnum2id.json')
    ...
    DataProcessor.generate_datajson('.../data.json', 50)
    DataProcessor.dump_json('.../user.json', mode='user')
    DataProcessor.dump_json('.../jd.json', mode='jd')
    DataProcessor.dump_json('.../graph_hired_jd.json', mode='graph_jd')
    DataProcessor.dump_json('.../graph_hired_user.json', mode='graph_user')
    DataProcessor.dump_json('.../graph_nothired_jd.json', mode='graph_nothired_jd')
    DataProcessor.dump_json('.../graph_nothired_user.json', mode='graph_nothired_user')

    #This only used when split the train test data
    '''
    with open('D:/FTEC5910/cvjd_parser/data.json', 'r') as file:
        data = json.load(file)
    ...
    # Write the train data to a separate JSON file
    with open('D:/FTEC5910/cvjd_parser/train-test_data/data_train.json', 'w') as file:
        json.dump(train_data, file)

    # Write the test data to a separate JSON file
    with open('D:/FTEC5910/cvjd_parser/train-test_data/data_test.json', 'w') as file:
        json.dump(test_data, file)

    # Write the dev data to a separate JSON file
    with open('D:/FTEC5910/cvjd_parser/train-test_data/data_dev.json', 'w') as file:
