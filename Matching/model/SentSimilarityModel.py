...
class InferSent(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.bsize = config['bsize'] #batch size, 4
        self.word_emb_dim = config['word_emb_dim'] #word embedding dimension,100
        self.enc_lstm_dim = config['enc_lstm_dim'] #2048
        self.pool_type = config['pool_type'] #max pool
        self.dpout_model = config['dpout_model'] #encoder dropout,0
        self.version = 1 if 'version' not in config else config['version']

        self.enc_lstm = nn.LSTM(self.word_emb_dim, self.enc_lstm_dim, 1,
                                bidirectional=True, dropout=self.dpout_model)
        # The following is the declaration of some special tokens. We don't have these tokens in data, so
...
class SentSimilarityModel(nn.Module):
    def __init__(self, config):
        ...
        # classifier
        self.nonlinear_fc = config['nonlinear_fc'] #use nonlinearity in fc
        self.fc_dim = config['fc_dim'] #512,nhid of fc layers
        self.n_classes = config['n_classes'] #2,0/1
        self.enc_lstm_dim = config['enc_lstm_dim'] #2048,encoder nhid dimension
        self.encoder_type = config['encoder_type'] #use inferSent
        self.dpout_fc = config['dpout_fc'] #classifier dropout,0.

        self.encoder = eval(self.encoder_type)(config) #InferSent
        self.inputdim = 4*2*self.enc_lstm_dim #4*2*2048
