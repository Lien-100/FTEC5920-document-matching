...
def get_optimizer(s):
    ...
    expected_args = inspect.signature(optim_fn).parameters
    expected_args= list(expected_args.keys())
    # check that we give good parameters to the optimizer
    if not all(k in expected_args[1:] for k in list(optim_params.keys())):
        raise ValueError("Error: Unexpected optimizer parameter provided")
